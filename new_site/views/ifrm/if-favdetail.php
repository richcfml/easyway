...
$("#clsAddtoCartTMP").die('click').live("click", function()
{
	...
	var mRandom = Math.floor((Math.random()*1000000)+1); 
	var url = "<?=$SiteUrl?><?=$objRestaurant->url?>/?item=cart&favoritesindex="+favindex+"&ajax=1&ifrm=cart&rndm="+mRandom;
	$.ajax
	({
		url: url,
		cache: false,
...
$("#addtofavTMP").die('click').live("click", function()
{
	...
	var mRandom = Math.floor((Math.random()*1000000)+1); 
	var url = "<?=$SiteUrl?><?=$objRestaurant->url?>/?item=cart&findex="+favindex+"&tip="+mTip+"&DM="+mDelMethod+"&ajax=1&ifrm=cart&rndm="+mRandom;
	$.ajax
	({
		url: url,
		cache: false,
