...
?>
	<div style="border: #e4e4e4 1px solid !important;" id="prd_0001212">
...
?>  
		<div class="flip">
      		<div id="edit_sign"><a href="#" id="prd_<?=$prod->prd_id?>" onclick="event.preventDefault();showPopup(<?= $prod->prd_id ?>, <?= $checkAtt->HasAssociates ?> , <?= $checkAtt->HasAttributes ?>,<?= $index ?>);" style="color:#8c1515;"><img border="0" src="../images/gray_edit.gif" height="14px"></a></div>
		    <div id="counting"><?= stripslashes(stripcslashes($prod->item_title)) ?></div>
		    <div id="minus_sign" class="remove_cart"><a href="<?=$SiteUrl?><?= $objRestaurant->url ."/"?>?item=cart&amp;index=<?=$index?>&ifrm=cart"><img src="../images/minus_sign.gif" width="14" height="14" border="0"></a></div>
...
{  	  
		...
		{
?>             
			<form method="post" name="form1" id="form1" action="?item=checkout&ifrm=checkout">
...
<?php  
		...
		else 
		{
			...
			{
				$mPost = $SiteUrl.$objRestaurant->url."/?item=checkout&ifrm=checkout";
       		}
		    else
       		{
				$mPost = $SiteUrl.$objRestaurant->url."/?item=login&ifrm=login";
       		}	
?>
			...
			<div class="online_ordering">
				<div style="width: 160px; margin: 10px auto;">
					<div style="float: left;">
						<img src="../images/store_closed.png" width="75" /> 
					</div>
					<div style="float: left; margin-left: 15px; text-align: center; color:#CC0000;">
						<?=  date('g:i A' , $objRestaurant->openTime)."<br />to<br />" . date('g:i A' , $objRestaurant->closeTime);?>
					</div>
					<br style="clear:both;" />
				</div>
			</div>
...
{
	var url=$(source).attr('href')+ '&ajax=1&ifrm=cart';
	$.ajax({
		url: url,
		type: "POST",
