...
else 
{
	...
	if(isset($_POST['x_order_id'])) 
	{
		...
		if($_POST['x_response_reason_code']==1)
		{
			...
			//@mysql_close($mysql_conn);	
			echo "<script type=\"text/javascript\">window.location='".$SiteUrl.$objRestaurant->url."/?item=thankyou&ifrm=thankyou';</script>";
			exit;
		}
		else 
		{
			//@mysql_close($mysql_conn);
			echo "<script type=\"text/javascript\">window.location='".$SiteUrl.$objRestaurant->url."/?item=failed&ifrm=failed&response_code=". $_POST['x_response_reason_code'] ."';</script>";
			exit;
		}
	}
?>
	<style type="text/css">
		#ContentImages{ width:800px; /*border:#999 solid 1px;*/ /*padding:50px*/;}
		#ImageOrderItsWay{ float:left; padding:20px 50px 20px 20px; border:#999 solid 1px; /*width:449px;*/}
		#ImageAddThis{ float:left; border:#999 solid 1px; /*width:455px;*/ margin-top:-19px; padding-top: 26px; padding-left: 40px; padding-right:30px; }
		#OrderInformationContent{ padding:15px; border:#999 solid 1px; width: 800px;}
		#TableInfo1{ padding-left:15px; padding-top:20px;}
		#Total{ font-size:13px; font-weight:bold; }
	</style>
	<div id="thank_you_area" style="padding:5px; float:none; width:800px !important;">
				...
				<tr>
					...
					<td style="text-align: center; vertical-align: middle; width: 50%;">
						<table style="width: 100%;">
							<tr>
								<td style="padding-top: 26px; padding-left: 20px;">
									<?php 
												...
												<div style="margin: 0; padding: 0;">
													<script type="text/javascript" language="javascript">
														$("#addfavourites").live('click',function(e) 
														{
															...
															var mCardCount = <?php echo(count($loggedinuser->arrTokens)); ?>;
															if (mCardCount<1)
															{
																$('#spnError1').show();
																return false;
													...
													<span id="spnError1" style="display: none; color: red; font-size: 12px;">Enter a credit card before adding an item as favorite.</span>
													<?php 
												...
												<script language="javascript" type="text/javascript">
													$(document).ready(function()	
													{
														$("#hrefLM").click(function() 
														{
															...
															{ 
																jQuery.facebox({div: '?mod=resturants&item=reordercm&ajax=1&ifrm=resturants'});  
																$('#facebox').center();
																return false;  
															});
															...
															{ 
																jQuery.facebox({div: '?mod=resturants&item=reordermobile&ajax=1&ifrm=resturants'});  
																$('#facebox').center();
																return false;  
															});
													...
													<div style="width: 650px !important; padding:10px;font-weight:normal;font-size:11px; display:none;"  class="repidreoderingterms" id="repidreoderingterms" name="repidreoderingterms">
	...
	?>
		<span style="margin-top: 15px;">
			...
			<a href="<?=$SiteUrl. $objRestaurant->url ?>/?ifrm=load_resturant"><strong>Go to Home Page</strong></a> <br />
			<br />
			<br />
		</span>	
	<?php
	...
	?>
		<span style="margin-top: 15px;">
			...
			<a href="<?=$SiteUrl. $objRestaurant->url ?>/?ifrm=load_resturant"><strong>Go to Home Page</strong></a> 
			<br />
			<br />
		</span>
	...
	<?php
	$mSQL = "SELECT COUNT(*) AS AnalyticsCount FROM `analytics` WHERE resturant_id=".$objRestaurant->id;
	$mRes = dbAbstract::Execute($mSQL);
	if (dbAbstract::returnRowsCount($mRes)>0)
	{
		$mRow = dbAbstract::returnObject($mRes);
		if ($mRow->AnalyticsCount>0) //Analytics Record exists
		{
			$mSQL = "UPDATE `analytics` SET fb_orders=fb_orders+1, fb_orders_amount=fb_orders_amount+".$cart->grand_total()." WHERE resturant_id=".$objRestaurant->id;
                        dbAbstract::Update($mSQL);
                }
		else //Analytics Record do not exists
		{
			$mSQL = "INSERT INTO `analytics` (resturant_id, fb_orders, fb_orders_amount) VALUES (".$objRestaurant->id.", 1, ".$cart->grand_total().")";
