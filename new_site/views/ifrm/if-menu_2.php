...
if (isset($_GET['faverr'])) {
    echo("<script>jQuery.facebox({div: '?item=addtofavorite&id=" . $_GET["id"] . "&ajax=1&err=1&ifrm=addtofavorite'});</script>");
}
...
<div id="body_left_col" style="width: 60% !important;">
    <?php
    $firstindex = 0;
    $current_cat1 = 0;
    while ($loop_index < 1) {
        ...
        foreach ($menus_details as $menulist) {
            if ($loop_index_check == FALSE) {
                ...
                <div class="left_col_inner_block" style="width: 100% !important;" <? if ($loop_index == 1) { ?>  <? } ?>>
                    <?php
                ...
                if ($current_cat1 != $menulist->sub_cat_id || $firstindex1 == 0) {
                    ...
                    <div class="left_col_inner_block" style="width: 100% !important;">
                        <?php
                    }
                                    ...
                                    <td align="left" style="width: 87%; font-size:12px;">
										...
										?>
                                        	<div class="product_name">
                                            	<a class="<?php echo('pr-'.$menulist->prd_id); ?>" <?php if ($iscurrentMenuAvaible == 0) { ?> href="javascript:alert('menu is not available at this time');" <?php } else { ?> href="#" id="prd_<?=$menulist->prd_id?>"  onclick="event.preventDefault();showPopup(<?php echo $menulist->prd_id; ?> , <?php echo $menulist->HasAssociates; ?>, <?php echo $menulist->HasAttributes; ?>, -1);" <? } ?> ><?= stripslashes($menulist->item_title) ?></a><br />
                                                <span style="font-size:12px;"><? echo stripslashes(stripcslashes($menulist->item_des)) ?></span>
                                            </div>
                                        <? } else { ?>
                                            <div class="product_name">
                                                <a class="<?php echo('pr-'.$menulist->prd_id); ?>"  onMouseover="ddrivetip('<span><?= htmlspecialchars(trim(str_replace("&#39;", "'", str_replace("<br />", "\n", str_replace("<br/>", "\n", str_replace("<br>", "\n", $menulist->item_title)))))) ?></span><br /><br /><?= htmlspecialchars($function_obj->_esc_xmlchar(trim(str_replace("&#39;", "'", str_replace("<br />", "\n", str_replace("<br/>", "\n", str_replace("<br>", "\n", str_replace("&#174;", "®", str_replace("&#228;", "ä", str_replace("&#232;", "è", str_replace("&#241;", "ñ", $menulist->item_des))))))))))) ?>')" onMouseout="hideddrivetip()" <?php if ($iscurrentMenuAvaible == 0) { ?>  href="javascript:alert('menu is not available at this time');" <? } else { ?> href="#"  id="prd_<?=$menulist->prd_id?>" onclick="event.preventDefault();showPopup(<?php echo $menulist->prd_id; ?> , <?php echo $menulist->HasAssociates; ?>, <?php echo $menulist->HasAttributes; ?>, -1);" <? } ?> ><?= stripslashes($menulist->item_title) ?></a>
                                            </div>
                                        <? } ?>
...
<div id="body_right_col" style="float:right;border: 0px !important; margin-left:0px !important;">
    <script language="javascript" type="text/javascript">
                function hideShowDeliveryMethod(val) {
                        if( val == "delivery" ) {
                                window.location.href="?item=detail&method="+val + "&ifrm=detail";
                        } else if( val == "pickup" ) {
                                window.location.href="?item=detail&method="+val + "&ifrm=detail";	
                        }
                }
                ...
                function PlaceOrder(subtotal, min_order, receiving_method) {
                        ...
                        if(subtotal >= minitotal || receiving_method == 'pickup'){
                                        if (sess_val!="" || item_val=="logincart") {
                                                document.form1.action="?item=confirmorder&ifrm=confirmorder";
                                        } else {
                                                document.form1.action="?item=logincart&ifrm=logincart";
                                        }
							
                                        if(receiving_method != 'disabled' && receiving_method == '' ) {
                                                alert('Please select Pickup or Delivery');
                                document.form1.action = "?item=detail&ifrm=detail";
                            } else {
                    ...
                    function UpdateCart() {
                        document.form1.action = "?mod=resturants&item=updatecart&ifrm=cart";
                        document.form1.submit();
                    }

                    // Go Back where this page was previousely called.
                    function ReturnBack() {
                        window.location.href = "?mod=resturants&item=product&ifrm=product";
                ...
                <tr>
                    <td id="rewardwrap">
                        ...
                        <div style="float: right; margin: 10px 7px; width: 150px;">
                            <a style="text-decoration:none" rel="facebox" href="?item=valutec&ajax=1&ifrm=valutec">Click here to register your VIP card</a>
                        </div>
                        <div class="clear"></div>
                    </td>
                </tr>
    ...
    <div id="cart" style="border: 0px !important;">
        <?php
        	require_once($site_root_path . "views/ifrm/if-cart.php");
        ?>
    </div>
</div>
<div style="clear:left"></div>
