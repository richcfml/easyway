<?php 
	...
	***********************************************************************************/
	...
	foreach ($cart->products as $product)
	{
		...
		<strong>Item Total Price - </strong>'.$currency. round($product->sale_price * $product->quantity,2) .'<br>
		<hr><br>';
	}
	...
	{
		//@mysql_close($mysql_conn);
		redirect($SiteUrl .$objRestaurant->url ."/?item=thankyou&ifrm=thankyou&orderid=".$cart->order_id );
		exit;
	}
?>