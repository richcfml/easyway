...
if (isset($_POST['login'])) 
{
	...
	else 
	{
		...
		{
			redirect($SiteUrl .$objRestaurant->url ."/?item=menu&wp_api=checkout" );exit;
		}
	}
} 
				...
				<table style="width: 100%; margin: 0px;" border="0" cellpadding="0" cellspacing="0" align="center">
											...
											success: function(data)
													 {
														...
														else if (mResult>0) //EWO Associated Account
														{
															...
															if (itemCount<=0)
															{
																window.location ="<?php echo($SiteUrl.$objRestaurant->url); ?>/?item=load_resturant&wp_api=load_resturant";
															}
															else
															{
																window.location ="<?php echo($SiteUrl.$objRestaurant->url); ?>/?item=menu&wp_api=checkout";
															}
														}
													 },
					...
					<tr>
						<td style="font-size: 12px;">
							Forget your password? <a href="?item=forgotpassword&wp_api=forgotpassword" style="color:#03F; font-size:12px;">Click Here</a>
						</td>
					</tr>
					<tr style="height: 10px;">
						<td>
						   ...
						   <div class="msg_warning"> Sorry, registeration failed please try again.</div>
							  ...
							  <? } ?>
							   <? if ($register_result===0){?>
									<div class="msg_warning"> This email is already registerd with easywayordering.com/<?= $objRestaurant->url ?>. Please <a href="<?=$SiteUrl?><?= $objRestaurant->url ."/"?>?item=forgotpassword&wp_api=forgotpassword">Click Here</a> if you forgot your passowrd.</div>
