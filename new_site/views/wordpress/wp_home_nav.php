...
?>&nbsp;&nbsp;<a class="<?=$class?>"  title="<?= stripslashes($menu->menu_name) ?>"  menuid="<?=$menu->id ?>" timings='<?= $menu->openTime ." to ". $menu->closeTime  ?>' href="<?=$SiteUrl?><?= $objRestaurant->url ."/"?>?menuid=<?=$menu->id ?>&wp_api=load_resturant"><?= stripslashes($menu->menu_name) .
 ($isOpen==0 ?"[". $menu->openTime ." to ". $menu->closeTime ."]":"") ?></a>
 &nbsp;&nbsp;<?  if($i < count($menus)-1) echo "|&nbsp;&nbsp;"; }?>    
  </div>
  <? if (isset($loggedinuser->id)){ ?>
  <span style="float:right; padding-right:10;" class="generaltext2"><a  href="?item=account&wp_api=account" <?  if(isset($item) && $item == 'accountdetail') echo "class='selected_red'" ?>>My Account</a>&nbsp;&nbsp;Welcome <? echo $loggedinuser->cust_your_name?>&nbsp;&nbsp;&nbsp;&nbsp;<a href="?item=logout&wp_api=logout">Logout</a></span>
  <? } else {?>
  <span style="float:right; padding-right:10;"><a href="?item=login&wp_api=login">Login/Register</a></span>
  <? }?>
</div>