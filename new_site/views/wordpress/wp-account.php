<?php 
	if(!is_numeric($loggedinuser->id)) {
		 ...
		 header("location: ". $SiteUrl .$objRestaurant->url ."/?item=login&wp_api=login" );exit;
	}
...
?>
<div id="body_left_col" style="width:66.5%; border:#e4e4e4 1px solid;">
      ...
      <div class="account_detail"> <strong>Email:</strong>&nbsp;&nbsp;
        ...
        &nbsp;[&nbsp;<a  href="?item=editaccount&wp_api=editaccount">Change</a>&nbsp;]<br />
        <br />
        <span style="font-weight:bold;" >Main Contact Information </span> [&nbsp;<a href="?item=editaccount&wp_api=editaccount" class="RedText_Underline">edit</a>&nbsp;]<br />
...
?>
        ...
        <br />
        <br />
        <strong>Alternate Delivery Address:</strong> [&nbsp;<a href="?item=editaccount&wp_api=editaccount" class="RedText_Underline">edit</a>&nbsp;]<br />
        ...
        <br>
        <form action="?item=checkout&wp_api=checkout" method="post" name="form1">
	...
	$i=0;
		...
		?>
          <tr style="background-color:<?=$colour?>">
            <td align="left" valign="middle"><a href="?mod=<?=$mod?>&item=orderhistory&Orderid=<?=$orderid?>&wp_api=orderhistory" class="RedText_Underline">
              <?=$userOrderRs->OrderID?>
              </a></td>
