...
<?php
	...
	{
		 redirect($SiteUrl .$objRestaurant1->url ."/?item=login&wp_api=login");
		 exit;
	}
			...
			if (is_numeric($$attribute_name) || is_array($$attribute_name)) 
			{
				if (is_array($$attribute_name)) 
				{
					...
					while ($inner_index < count($arr)) 
					{
						...
						//preg_replace("/[^0-9+-.]+/", "", $attribute->Price);
						if ($attribute->Price == '')
						{
							$attribute->Price = 0;
						}
	...
	<?php
		...
		if (dbAbstract::returnRowsCount($mAttributeRes)>0)
		{
			...
			while ($mAttributeRow = dbAbstract::returnObject($mAttributeRes))
			{
				...
				$mPriceAtt =  $mAttributeRow->Price; 
				$mPriceDisp = $mPriceAtt;
				if ($mPrice!='0' || $mPrice!='0.00') 
				{
					if ($mPriceAtt<0) 
					{
						$mPriceMinus = $mPriceMinus + $mPriceAtt;		  
						$mPriceDisp = " - Subtract ".$currency.$mPriceAtt;
					} 
					else 
					{
						$mPricePlus = $mPricePlus + $mPriceAtt;
						$mPriceDisp = " - Add ".$currency.$mPriceAtt;
					}
				}
				else
				{
		...
		?>
			...
			<td style="width: 25%;">
		<?php 
				$itemTotalPrice = $mPrice + $Tot_atrib_price_Plus + $assocTotalPrice + $Tot_atrib_price_mines;
				$Tot_atrib_price_Plus = 0;
		?>
		...
		<td>
			<div style="width: 25%;">
				<div style="float: left;">
					<form id="myForm" name="myForm" method="post" action="?item=orderhistory&wp_api=orderhistory&Orderid=<?=$mOrderID?>">
				...
				<div style="display: none; border-radius:7px;">
					...
					<form id="myForm1" name="myForm1" method="post" action="?item=orderhistory&wp_api=orderhistory&Orderid=<?=$mOrderID?>">
