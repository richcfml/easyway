 <form id="checkoutform" name="checkoutform" action="<?php echo $SiteUrl.$objRestaurant->url; ?>/?item=confirmpayments&session_id=<?= session_id() ?>&wp_api=confirmpayments" method="post">
	<? include "wp-cart_products.php" ?> 
    <div id="second_body">
        <div id="second_body_col" style="width:650px; float:left;">
      	<? include "wp-customer.php" ?>
            <div class="second_body_heading">Special Requests/Notes</div>
            <div class="text_area">
                <textarea name="special_notes" id="special_notes"></textarea>
            </div>
        <? include "wp-valutec.php" ?>
            <!-- RESTAURANT USE VIP CARD --> 
        </div>
        <div class="clear"> </div>
    </div>
         <? include "wp-paymentmethods.php" ?>
...
function drawzones(restaurantlocation) {        
    ...
    if ($('input:radio[name=payment_method]:checked').val() == 1){
        if(<?= $cart->delivery_type ?> != <?= cart::Pickup ?>)
        {
			...
			$("#checkoutform").attr('action', '<?php echo $checkoutProtocol.str_replace("https://", "", str_replace("http://", "", $SiteUrl)).$objRestaurant->url;?>/?item=confirmpayments&session_id=<?= session_id() ?>&wp_api=confirmpayments');
		 }
		 else
		 {
			  $("#checkoutform").attr('action','?item=submitorder&wp_api=submitorder');
		 }
			$("#checkoutform").submit();
...
$(function() {
    ...
    function add_delivery_tip(tip) {
        ...
        $.post('?item=addtip&ajax=1&wp_api=addtip', {tip: tip, action: "updatetip"}, function(data) {
    ...
    function redeemCoupon() {
        var coupon_code = $("#coupon_code").val();
					$.post('?item=redeemcoupon&ajax=1&wp_api=redeemcoupon',{coupon_code:coupon_code,action:"redeemcoupon"},function(data) {
            $(".cart_statsarea").html(data);
        });
    }
    ...
    function vipredeemreward() {
	    ...
	    $.post('?item=redeemcoupon&ajax=1&wp_api=redeemcoupon',{vipreward:vipreward,action:"vipreward"},function(data) {
            $(".cart_statsarea").html(data);
