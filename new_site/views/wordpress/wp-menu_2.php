                            ...
                            <table style="margin: 0px; width: 100%;" cellpadding="0" cellspacing="0" border="0">
                                <tr style="height: 40px;">
                                    ...
                                    <td align="left" style="width: 87%; font-size:12px;">
                                        ...
                                        <? } else { ?>
                                            <div class="product_name">
                                                <a id="prd_<?=$menulist->prd_id?>" class="<?php echo('pr-'.$menulist->prd_id); ?>"  onMouseover="ddrivetip('<span><?= htmlspecialchars(trim(str_replace("&#39;", "'", str_replace("<br />", "/n", str_replace("<br>", "\n",str_replace("<br/>", "\n", $menulist->item_title)))))) ?></span><br /><br /><?= htmlspecialchars($function_obj->_esc_xmlchar(trim(str_replace("&#39;", "'", str_replace("<br />", "/n", str_replace("<br>", "\n",str_replace("<br/>", "\n", str_replace("&#174;", "®", str_replace("&#228;", "ä", str_replace("&#232;", "è", str_replace("&#241;", "ñ", $menulist->item_des))))))))))) ?>')" onMouseout="hideddrivetip()" <?php if ($iscurrentMenuAvaible == 0) { ?>  href="javascript:alert('menu is not available at this time');" <? } else { ?> href="#" onclick="event.preventDefault();showPopup(<?php echo $menulist->prd_id; ?> , <?php echo $menulist->HasAssociates; ?>, <?php echo $menulist->HasAttributes; ?>, -1);" <? } ?> ><?= stripslashes($menulist->item_title) ?></a>
                                            </div>
                                        <? } ?>
