...
<div style="font-size:14px; font-weight:bold; margin:3px 0px 0px 174px;"> <a href="?item=menu&wp_api=load_resturant" style="text-decoration:none; color:#CC0000;">Change to
                    ...
                    $arr_index=0;
                     foreach($prod->attributes as $attr) { 
                      ...
                      echo " <strong>".$attr->Option_name .":</strong>"
                        ?>
      <? } echo ($arr_index>0 ? ", ":"") . $attr->Title ." ".  ($attr->Price=='0' ?'' : ($attr->Price[0]=='-' ? ' - Subtract': ' - Add'   ).$currency.preg_replace_callback("/[^0-9.]+/", function ($matches) { return ''; }, number_format($attr->Price,2)) /*preg_replace("/[^0-9.]+/","",number_format($attr->Price,2))*/) ;?>
      <? $arr_index +=1;
                          }
                      // FO RACH ATTR 
