...
class chargifyMeteredUsage 
{
    ...
    {
        $mSQL = "SELECT c.api_access_key,r.chargify_subscription_id,c.hosted_page_url,c.credit_card_orders_quantity,c.credit_card_orders_amount,c.cash_orders_quantity,c.cash_orders_amount ,c.rapid_reorder_text_messages FROM resturants r LEFT JOIN chargify_products c ON r.chargify_product_id=c.settings_id AND c.api_access_key!='' WHERE r.id=$restaurant_id";
        $resturants = dbAbstract::Execute($mSQL);
        if(dbAbstract::returnRowsCount($resturants, 0) > 0) 
        {	
            $resturant = dbAbstract::returnAssoc($resturants, 0);
