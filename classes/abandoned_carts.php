...
class abandoned_carts 
{
    ...
    public function make_current_order_abandoned($id) 
    {
        ...
        $mSQL = "UPDATE `abandoned_carts` SET `status`=1 WHERE id=$id";
        dbAbstract::Update($mSQL); 
        Log::write("Delete abandoned_carts - abandoned_carts.php", "QUERY -- ".$mSQL, 'order', 0 , 'user');
        $this->unset_sessions();
    }
    ...
    {
        $mSQL = "UPDATE `analytics` SET `abandoned_carts_count_last_month`=$orders_count WHERE resturant_id=$resturant_id";
        dbAbstract::Update($mSQL);    
    }
