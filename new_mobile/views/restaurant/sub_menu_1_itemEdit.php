...
<!--                    <article data-onclick="showPopup(173926, 0, 0, -1, this)" class="menu-item editPrd ">
    ...
    <script>
        ...
        function showPopup(productId, hasAssociates, hasAttribute, cartItemIndex) {
            ...
            $('#edit_item #totalattributes').val(0);
                ...
                $.ajax
                        ({
                            ...
                            success: function (data)
                            {
                                ...
                                console.log('1'+$('  #edit_item  #retail_price').html())
                                ...
                                console.log('2'+$('  #edit_item  #retail_price').html())
                                var requiredIndex = 0;
                                $.each(attributes_array, function (i, attribute)
                                {
                                    ...
                                    if (($.trim(mLimit) != "") && ($.trim(mLimitPrice) != ""))
                                    {
                                        if ((mLimit > 0) && (mLimitPrice > 0))
                                        {
                                            ...
                                            {
                                                mStrRe = mStrRe + '$' + mLimitPrice + ' will be added for each additional selection';
                                            }
                                            else
                                            {
                                                mStrRe = mStrRe + '&nbsp;&nbsp;-&nbsp;&nbsp;($' + mLimitPrice + ' will be added for each additional selection)';
                                ...
                                if (association_array != '') {
                                    ...
                                    if (associationResult.length > 0) {
                                        ...
                                        $.each(associationResult, function (i, item) {
                                            ...
                                            var ind_val = index + 1;
                                            html = html + '<td class="attribute"><input class="clsAssoc" id="' + item[productId]['0'].prd_id + '" price="' + item[productId]['0'].retail_price + '" type="checkbox" ' + selected + ' name="associations[]" value="' + ind_val + '" >' + item[productId]['0'].item_title + '&nbsp;<span class="red">$' + item[productId]['0'].retail_price + '</span></td>'
                                            index = ind_val;
                                            if (index % 3 == 0) {
                                                html = html + '</tr>';
                                            }
            ...
            window.location.hash = '#edit_item', EasyWay.Notification.open()
        }
        ...
        $(document).ready(function ()
        {
            ...
            $(document).on('change', '.clsAssoc', function ()
            {
                ...
                if ($.trim(mPrice) != "")
                {
                    ...
                    mPreviousPrice = mPreviousPrice.toFixed(2);
                    $('  #edit_item  #retail_price').html("<?= $currency ?>" + mPreviousPrice);
                }
            });
        ...
        function  add_to_cart(elementt) {
            ...
            var product_title = $('  #item_title').text();
            var product_quantity = $(' #quantity').val();
            var product_sale_price = $('  #edit_item  #retail_price').text().replace('$', '')
