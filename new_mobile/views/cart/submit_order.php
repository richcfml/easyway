<?php
	...
	{
		if($_POST['serving_date_time']=='As soon as possible') 
		{
			$asap=1;
		}else{
			$serving_date = date('Y-m-d H:i', strtotime($_POST['serving_date_time']));
		}
	...
	$payment_method=($_POST['method'] == 'credit_card')? "Credit Card" : "Cash";
	...
	$cart->payment_menthod=($_POST['method'] == 'credit_card')? 1 : 2;;
	...
	***********************************************************************************/
	...
	$arrRestaurant['DEVLIVERYDATETIME'] = ($asap==1)? 'as soon as possible':$serving_date;
	...
	***********************************************************************************/
	...
	{
		$mSQL = "UPDATE grouporder SET Status=1, FoodOrdered=1 WHERE GroupID=".$_GET["grpid"]." AND CustomerID=".$_GET["grp_userid"];
		dbAbstract::Update($mSQL);
		
		redirect($SiteUrl .$objRestaurant->url ."/?item=thankyou&orderid=".$cart->order_id."&grp_userid=".$_GET["grp_userid"]."&grpid=".$_GET["grpid"]."&uid=".$_GET["uid"]."&grp_keyvalue=".$_GET["grp_keyvalue"]);
	}
	else
	{
