...
include("../../../includes/config.php");
	...
	if($_REQUEST['apply_subcat']) {
		...
		while($prodRs = dbAbstract::returnArray($prodQry, 1)){
                        Log::write("Delete product association - add_coupon_items.php", "QUERY -- DELETE FROM product_association where product_id = '".$prodRs['prd_id']."'", 'menu', 1 , 'cpanel');
		...
		while($prodRs = dbAbstract::returnArray($prodQry, 1)){
			...
			for($i=0; $i<count($_GET['itemcheck']); $i++) {
				...
				$query = "INSERT INTO product_association SET product_id = '".$prodRs['prd_id']."', association_id = '".$association_id[$i]."'";
                                Log::write("Add new product association - add_coupon_items.php", "QUERY -- ".$query, 'menu', 1 , 'cpanel');
				dbAbstract::Insert($query, 1);
                                Log::write("Set product HasAssociates=1 - add_coupon_items.php", "QUERY -- UPDATE product set HasAttributes=1 WHERE prd_id = " . $prodRs['prd_id'] . "", 'menu', 1, 'cpanel');
                                dbAbstract::Update("UPDATE product set HasAssociates=1 WHERE prd_id = " . $prodRs['prd_id'] . "", 1);
            }
			}// end for
		}// end while
	} else {
		Log::write("Delete product association - add_coupon_items.php", "QUERY -- DELETE FROM product_association where product_id = '".$_GET['product_id']."' ", 'menu', 1 , 'cpanel');
	  ...
	  for($i=0; $i<count($_GET['itemcheck']); $i++) {
		  $query = "INSERT INTO product_association SET product_id = '".$_GET['product_id']."', association_id = '".$association_id[$i]."'";
                  Log::write("Add new product association - add_coupon_items.php", "QUERY -- ".$query, 'menu', 1 , 'cpanel');
		  dbAbstract::Insert($query, 1);
                Log::write("Set product HasAssociates=1 - add_coupon_items.php", "QUERY -- UPDATE product set HasAttributes=1 WHERE prd_id = " . $_GET['product_id'] . "", 'menu', 1, 'cpanel');
