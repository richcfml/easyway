<?php
	if(!isset($repid_payment))
	{
		...
		{
			redirect($client_path .$objRestaurant->url ."/?item=cart" );
			exit;
		}
		...
		{ 
			...
			redirect($client_path .$objRestaurant->url ."/?item=menu" );
			exit;
		}
	}
	//--------Edited by Asher Ali------------
	...
	//echo $total<=0; exit;
	//redirect($client_path .$objRestaurant->url ."/?item=cart" );exit;
	////header("location: ". $client_path .$objRestaurant->url ."/?item=cart" );exit;
	//}
	//---------------------------------------------//
	$is_guest = 0;
	...
	***********************************************************************************/
	...
	foreach ($cart->products as $product)
	{
		...
		$common_confimration_email.='<br/><strong>Special Notes - </strong>'. $product->requestnote .'<br><br>
									 <strong>Each Item Price - </strong>'.$currency. $product->retail_price .'<br>
									 <strong>Item Total Price - </strong>'.$currency. ($product->sale_price * $product->quantity ) .'<br>
									 <hr><br>';
	}
	...
	<a href=\"http://easywayordering.com/ozzies/\">http://www.easywayordering.com/'. $objRestaurant->url .'/</a><br>Phone: '. $objRestaurant->phone .'<br>Fax: '. $objRestaurant->fax .'<br>';
	...
	if($objRestaurant->phone_notification == 1) 
	{
		...
		$objMail->sendTo(addslashes($phone_notification_mail_body), '',$voice_email_address,false);
	}
	...
	//-------------------------------------//
	redirect($client_path .$objRestaurant->url ."/?item=thankyou" );
?>
