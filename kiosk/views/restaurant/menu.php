...
<script>
    ...
    function setProductDetailsToPopup(productDetails){
		...
		$('#lblEndPrice').html('<?=$currency?>' + productDetails.retail_price);
	...
	function showPopup(productId,hasAssociates,hasAttribute,cartItemIndex) {
        ...
        if(hasAssociates == 1 || hasAttribute == 1 || cartItemIndex>=0){
            ...
            {
                mUrl = "<?= $SiteUrl ?><?= $objRestaurant->url ?>/?item=favindex&kiosk=1&showpopup=1&ajax=1";
            }
            $.ajax
            ({
                url: mUrl,
                ...
                success: function(data)
                {
                    ...
                    if(data.hasOwnProperty('quantity')){
						...
						$(".tmpClsQ").html(quantity);
						$("#txtPrevQ").val(quantity);
                    }else
                    {
                        $('#quantity').val(1);
						$(".tmpClsQ").html(1);
...
$.each(attributeRequired, function(index, value) {
  ...
  $.each(filteredAttributeElements, function(index, element) {    
    ...
    else
    {
        ...
        mUrl = "<?= $SiteUrl ?><?= $objRestaurant->url ?>/?item=favindex&kiosk=1&addtocart=1&ProductID=" + product_id + "&rndm=" + mRandom + "&ajax=1";
        $.facebox.close();
        $.ajax
        ({
            url: mUrl,
            ...
            success: function(data)
            {
                ...
                '</div>';
                    ...
                    $("#cart").load("<?= $SiteUrl ?><?= $objRestaurant->url ?>/?item=cart&kiosk=1&ajax=1");
                                ...
                                <tr style="height: 40px;">
                                    ...
                                    <td align="left" style="width: 88%; font-size:12px;" valign="top">
                                        <script type="text/javascript" language="javascript">
                                            function menuNotAvailable()
                                            {
                                                swal({
                                                    title: "",
                                                    text: "<span style='font-size: 17px; color: #575757; font-weight: 500 !important;'>Menu is not available at this time.</span>", 
                                        ...
                                        <div class="product_name"><a id='showProductPopup'  <? if ($iscurrentMenuAvaible == 0) { ?>  href="javascript:menuNotAvailable();" <? } else { ?> href="#"  onclick="event.preventDefault();showPopup(<?php echo $menulist->prd_id; ?> , <?php echo $menulist->HasAssociates; ?>, <?php echo $menulist->HasAttributes; ?>,-1);"<? } ?> ><?= stripslashes($menulist->item_title) ?></a></div>
                                    </td>
...
<div id="body_right_col" style="float:right;">
    <script language="javascript" type="text/javascript">
        function hideShowDeliveryMethod(val)
        {
            if (val == "delivery")
            {
                window.location.href = "?item=detail&kiosk=1&method=" + val;
            }
            else if (val == "pickup")
            {
                window.location.href = "?item=detail&kiosk=1&method=" + val;
        ...
        function PlaceOrder(subtotal, min_order, receiving_method)
        {
            ...
            if (subtotal >= minitotal || receiving_method == 'pickup')
            {
                ...
                {
                    document.form1.action = "?item=confirmorder&kiosk=1";
                }
                else
                {
                    document.form1.action = "?item=logincart&kiosk=1";
                }
                ...
                if (receiving_method != 'disabled' && receiving_method == '')
                {
                    ...
                    document.form1.action = "?item=detail&kiosk=1";
                }
                else
                {
            ...
            else if (subtotal < minitotal && receiving_method == 'delivery')
            {
                swal({
                    title: "",
                    text: "<span style='font-size: 17px; color: #575757; font-weight: 500 !important;'><?=$java_currency?>" + restMinTotal + " of food required to checkout. <br />Please add more items.</span>",
                    confirmButtonColor: "#11b1b3",
                    html: true
                });
            }
        ...
        function UpdateCart()
        {
            document.form1.action = "?mod=resturants&item=updatecart&kiosk=1";
            document.form1.submit();
        }

		function updatePrice()
		{
			var mVal = $(".qnty:last").val();
			
			if (isNumeric(mVal))
			{
				mTmp = $('.tmpCls:last').html();
				mTmp = mTmp.replace("<?=$currency?>", "");
				mTmp = mTmp.replace(/\u00A3/g, "");
				mTmp = mTmp*mVal;
				mTmp = mTmp.toFixed(2);
				mTmp = "<?=$currency?>"+mTmp;
        ...
        function ReturnBack()
        {
            window.location.href = "?mod=resturants&item=product&kiosk=1";
        }//function
                                ...
                                <table style="width: 100%; margin: 0px;" cellpadding="0" cellspacing="0" border="0">
                                    <tr>
                                        ...
                                        <td style="font-size: 11px; width: 45%;margin-right: 2px !important;" valign="middle" align="left">
                                            <a style="text-decoration:none" rel="facebox" href="?item=valutec&kiosk=1&ajax=1">Click here to register your VIP card</a>
                                        </td>
                                    </tr>
                                </table>
                                <div class="clear"></div>
        ...
        <div style="width: 65%; height: 80px;">
            ...
            <div style="float: left; margin-top: 9px;" id="item_detail_wrapper">
                ...
                <div style="margin-top: 6px;"> 
                    <strong>Item Price:</strong> <span style="font-size:12px; color:#ff0000;" id="retail_price" class="tmpCls"></span><span style="font-size:12px; color:#ff0000;">&nbsp;x&nbsp;</span><span style="font-size:12px; color:#ff0000;" id="lblQuantity" class="tmpClsQ">1</span><span style="font-size:12px; color:#ff0000;">&nbsp;=&nbsp;</span><span style="font-size:12px; color:#ff0000;" class="tmpClsEp" id="lblEndPrice"></span>
                </div>
            </div>
            <br>
        </div>
				...
				$(document).ready(function () 
				{
					...
					$(".qnty").keyup(function (e) 
					{
						...
						if (isNumeric(mVal))
						{
							...
							mTmp = mTmp.replace("<?=$currency?>", "");
							mTmp = mTmp.replace(/\u00A3/g, "");
							if (isNumeric($("#txtPrevQ").val()))
							{
								mTmp = (mTmp/($("#txtPrevQ").val()));	
							}
							mTmp = mTmp*mVal;
							mTmp = mTmp.toFixed(2);
							mTmp = "<?=$currency?>"+mTmp;
