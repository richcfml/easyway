...
<div id="main_heading">
	<div id="nav_links">
		<?	
		$menu_qry	=	mysql_query("select * from menus where rest_id = ".$catid."");
		$menu_i= 0;
  ...
  ?>
    <td width="50%">
     ...
     <? if($sub_cat_status  == 1) {?>
       ...
       <a href="?mod=menus&item=submenustatus&catid=<?=$catid?>&submenuActivateId=<?=$sub_cat?>" onClick="return confirm('Are you sure you would like to change the status of  this Sub Menu?')"><img src="../images/enable.png" width="16" height="16" border="0" title="Disabled" /></a>
       <? }?>
      ...
      <?	$subcat_id	=	mysql_result($cat_qry,$count,"cat_id");
	  		$prd_qry	=	mysql_query("select * from product where sub_cat_id = $subcat_id");
      				...
      				while($prd_qryRs = mysql_fetch_object($prd_qry)){	?>
                                  ...
                                  &nbsp;&nbsp;<a href="?mod=menus&item=addattribute&amp;pid=<?=$prd_qryRs->prd_id?>&amp;catid=<?=$catid?>&amp;scid=<?=$sub_cat?>" class="smalllink">Add Attribute</a> <a href="admin_contents/menus/popup.php?catid=<?=$catid?>&pid=<?=$prd_qryRs->prd_id ?>" rel="facebox" class="smalllink">associate item</a>
                                  <?
