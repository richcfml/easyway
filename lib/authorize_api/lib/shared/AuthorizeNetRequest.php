...
abstract class AuthorizeNetRequest
{
    ...
    protected function _sendRequest()
    {
        ...
        parse_str($this->_post_string,$post_array);
        
        $c_panel = 0;
        $refund = '';
        if($post_array['x_type'] == 'CREDIT'){ $c_panel = 1; $refund = 'Refund';}
        Log::write('AuthorizeDotNet Post array - '. $refund, print_r($post_array,true), 'AuthorizeNet',$c_panel);
        ...
        if ($curl_error = curl_error($curl_request)) {
            Log::write('AuthorizeDotNet CURL error:', $curl_error, 'AuthorizeNet');
        }
       ...
       // echo $response;exit;
        ...
        //Log::write(__FILE__." : ", 'Authorize.net Response '.$response);
