...
#maincontainer {
	<? if($options["appearence"] == "1") { ?>
		min-width: 	<? echo ($options["min_width_of_the_container"] == "0px" ? "830px" : $options["min_width_of_the_container"]); ?> !important;
	<? } else { ?>
		min-width: 	<? echo ($options["min_width_of_the_container"] == "0px" ? "640px" : $options["min_width_of_the_container"]); ?> !important;
	<? } ?>
}
...
.left_col_inner_block {
	border-width: <? echo $options["cell_border_thickness"]; ?>px !important;	
 ...
 if($options["iframe_height_infinite"] == "1"){   ?>
	<script type="text/javascript">
		$(function(){
			...
			window.parent.postMessage('{"height":'+ height +'}', url);
		});
	</script>
<? } ?>
</head>
...
<? //echo $_SESSION["restaurant_deisgn_settings_id"];
if($_SERVER['SERVER_NAME'] == 'localhost') {
	$client_path = "/easywayordering/"; 
} else {
		$client_path = "http://www.easywayordering.com/";
}
?>
